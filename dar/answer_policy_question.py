...
class CallJournalUrls(LoadSpayModelSetUpPipeline):
    ...
    def get_doaj_articles(self, keywords1):
        ...
        DOAJ_articles = [self.process_doaj_article(article) for article in DOAJ_results]

        return DOAJ_articles

    def process_Crossref_article(self, item):
    ...
    def get_Crossref_articles(self, keywords1):
        ...
        Crossref_articles = [self.process_Crossref_article(
            item) for item in Crossref_items]

        return Crossref_articles

    def process_CORE_article(self, item):
    ...
    def call_CORE_api(self, page_CORE, query_CORE, api_key_CORE):
        """ Perform a single API call to CORE."""
        url_CORE = CallJournalUrls.base_url_CORE + query_CORE + "?page=" + str(page_CORE) + "&pageSize=" + str(
            100) + "&metadata=true&fulltext=false&citations=false&similar=false&duplicate=false&urls=false&faithful" \
                   "Metadata=false&apiKey=" + api_key_CORE
    ...
    def get_CORE_articles(self, keywords1):
        ...
        CORE_articles = [self.process_CORE_article(item) for item in CORE_list]

        return CORE_articles

    def check_for_claim(self, abstract_sentences):
    ...
    def missing_key_words(self, abstract, keywords):
    ...
    def filter_articles(self, keywords1, articles):
        ...
        for article in articles:
            ...
            if self.check_for_claim(abstract_sentences):
    ...
    def use_subjects_verbs_objects(self, sent_list, keywords1):
        ...
        while i != list_length:
            doc = LoadSpayModelSetUpPipeline.nlp(sent_list[i])
    ...
    def get_dependency_path_to_root(self, token, root):
    ...
    def get_dependency_path_between_tokens(self, token_a, token_b):
        """Traverse the path between relevant noun phrases in a sentence."""
        if token_b in token_a.subtree:
            all_tokens = self.get_dependency_path_to_root(token_b, token_a)
        elif token_a in token_b.subtree:
            all_tokens = self.get_dependency_path_to_root(token_a, token_b)
        else:
            ...
            tokens_a_side = self.get_dependency_path_to_root(
                token_a, lowest_common_ancestor)
            tokens_b_side = self.get_dependency_path_to_root(
    ...
    def use_noun_phrases(self, sent_list, keywords1):
        ...
        while i != list_length:
            ...
            if len(relevant_noun_phrases) == 2:
                tokens_btn_nps = self.get_dependency_path_between_tokens(
                ...
                if position is None:
                    ...
                    for token in tokens_btn_nps:
                        ...
                        if token.text in LoadSpayModelSetUpPipeline.opinion_words and str(token.tag_) == 'JJ':
                            position = True if token.text in LoadSpayModelSetUpPipeline.pos_words else False
    ...
    def assign_position(self, relevant_articles, keywords1):
        ...
        for item in relevant_articles:
            ...
            claims = self.get_claims(item['abstract'])
            item['claims'] = claims
    ...
    def get_article_positions(self, relevant_articles):
